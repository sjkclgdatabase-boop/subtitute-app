  ...
  <div class="p-8 max-w-7xl mx-auto min-h-screen space-y-8">
    ...
    <div class="bg-white rounded-3xl p-8 shadow-sm ring-1 ring-slate-900/5 space-y-2">
      ...
      <p class="text-slate-500 text-xs sm:text-sm font-medium leading-relaxed">
        配置学校作息参数、管理基础班级、设置MMI目标，支持上线前数据清理与备份。
      </p>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm ring-1 ring-slate-900/5 p-8 transition-all duration-300 hover:shadow-md space-y-6">
      ...
      <div class="space-y-6">
        <div>
          <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wider">当前学校名称</label>
          <input 
        ...
        <div>
          ...
          <div class="flex items-center gap-6">
            ...
            <div class="flex-1 space-y-2">
              ...
              <p class="text-[11px] text-slate-400 font-medium">支持 PNG, JPG 格式，图片会自动转换，小于 2MB 为佳。</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm ring-1 ring-slate-900/5 p-8 transition-all duration-300 hover:shadow-md space-y-6">
      ...
      <div class="space-y-6">
        <div>
          <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wider">每周上课天数 (天)</label>
          <div class="flex items-center gap-4">
            ...
            />
            <span class="text-xs text-slate-400 font-medium">例如：5（代表周一至周五）</span>
          </div>
        </div>

        <div>
          <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wider">每日总节数 (节)</label>
          <div class="flex items-center gap-4">
            <input 
              ...
              class="w-full sm:w-64 px-4 h-11 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
            />
            <span class="text-xs text-slate-400 font-medium">例如：11（上午班总节数）</span>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm ring-1 ring-slate-900/5 p-8 transition-all duration-300 hover:shadow-md space-y-6">
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-200 mb-6">
        <div>
          <h3 class="text-xs font-bold uppercase tracking-wider text-slate-950">批量导入/导出上课周历</h3>
          <p class="text-slate-500 text-xs mt-1 font-medium">支持下载标准CSV模板，填写后批量上传，适配周历数据格式。</p>
        </div>
        ...
        <div class="divide-y divide-slate-100 border border-slate-200 rounded-2xl overflow-hidden max-h-64 overflow-y-auto">
          ...
          <div v-for="w in allSchoolWeeks" :key="w.id" class="p-3.5 flex items-center justify-between bg-white hover:bg-slate-50 transition">
            <div class="flex items-center gap-4 text-xs font-bold text-slate-700">
              <span class="px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-xl font-bold">第 {{ w.week_number }} 周</span>
    ...
    <div class="bg-white rounded-3xl shadow-sm ring-1 ring-slate-900/5 p-8 transition-all duration-300 hover:shadow-md space-y-6">
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-200">
        <div>
          <h3 class="text-xs font-bold uppercase tracking-wider text-slate-950">导入/导出科目目标</h3>
          <p class="text-slate-500 text-xs mt-1 font-medium">支持 CSV 模板，上传后自动更新数据库目标配置。</p>
        </div>
        <div class="flex flex-wrap items-center gap-3">
    ...
    <div class="bg-white rounded-3xl shadow-sm ring-1 ring-slate-900/5 p-8 transition-all duration-300 hover:shadow-md space-y-6">
      ...
      <div class="space-y-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-slate-100 gap-4">
          <div>
            <h3 class="text-xs font-bold uppercase tracking-wider text-slate-950">清空所有请假与代课记录</h3>
            <p class="text-slate-500 text-xs mt-1 font-medium">仅删除请假、代课历史测试数据，保留教师资料与排课表。</p>
          </div>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-slate-100 gap-4">
          <div>
            <h3 class="text-xs font-bold uppercase tracking-wider text-slate-950">重置全校课表</h3>
            <p class="text-slate-500 text-xs mt-1 font-medium">清空全部排课数据，便于批量导入新学年课表（保留教师名单）。</p>
          </div>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h3 class="text-xs font-bold uppercase tracking-wider text-red-600">危险操作：初始化系统</h3>
            <p class="text-slate-500 text-xs mt-1 font-medium">一键清空请假、代课、课表、教师名单，恢复系统至初始空白状态。</p>
          </div>
