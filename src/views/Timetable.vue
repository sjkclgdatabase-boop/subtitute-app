    ...
    <div v-else class="bg-white rounded-3xl shadow-sm ring-1 ring-slate-900/5 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div class="overflow-x-auto p-6">
        <table class="w-full text-center border-collapse">
          <thead>
            <tr>
              <th class="p-4 text-slate-400 font-bold text-xs w-32 border-b-2 border-slate-100 uppercase">节次 \ 星期</th>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
        ...
        <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden ring-1 ring-slate-900/10 p-6 space-y-4">
          <div class="flex justify-between items-center mb-2">
            <div>
              <h2 class="text-base font-bold text-slate-900">编辑排课</h2>
              <p class="text-[11px] text-slate-500 mt-0.5 font-medium">星期{{ dayNames[editingData.weekday - 1] }} · 第 {{ editingData.period }} 节 ({{ getPeriodTime(editingData.period) }})</p>
            </div>
          ...
          <div class="space-y-3">
            <div>
              <label class="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wider">班级 (合班可用斜杠如: 3A/3B)</label>
              <input v-model="editingData.class_name" type="text" placeholder="例如: 3C 或 3A/3B" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all">
            </div>
            <div>
              <label class="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wider">科目</label>
              <input v-model="editingData.subject" type="text" placeholder="例如: 英文 或 PM" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all">
            </div>
          </div>
